...
import { getCategoryRanking } from '@/entities/transaction/lib/getCategoryRanking.tsx';

export const getInsights = (transactions: Transaction[], categories: Category[]): string[] => {
  ...
  if (incomeChange !== null) {
    ...
    insights.push(
      `Your income ${sign} by ${Math.abs(incomeChange).toFixed(1)}% compared to last month.`,
    );
  }
  if (expenseChange !== null) {
    const sign = expenseChange > 0 ? 'increased' : 'decreased';
    insights.push(
      `Your expenses ${sign} by ${Math.abs(expenseChange).toFixed(1)}% compared to last month.`,
    );
  }
  ...
  if (largestCategory) {
    insights.push(
      `Your largest spending category is "${largestCategory.categoryName}" with $${largestCategory.total}.`,
    );
  }
  ...
  if (topIncrease.length) {
    ...
    insights.push(
      `You spent ${maxInc.percentage.toFixed(1)}% more on "${maxInc.categoryName}" than last month.`,
    );
  }
  if (topDecrease.length) {
    const maxDec = topDecrease[0];
    insights.push(
      `You spent ${Math.abs(maxDec.change || 0).toFixed(1)}% less on "${maxDec.categoryName}" than last month.`,
    );
  }
